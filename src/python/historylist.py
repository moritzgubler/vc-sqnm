...
class HistoryList:
    ...
    def __init__(self, ndim, nhist_max, use_cupy = False):
        ...
        self.histList = self.np.zeros((ndim, nhist_max))
        self.diffList = self.np.zeros((ndim, nhist_max))
        self.normalizedDiffList = self.np.zeros((ndim, nhist_max))
        self.icount = 0
        self.firstFull = True
    ...
    def add(self, vectorToAdd):
        ...
        if self.icount < self.nhist_max:
            ...
            if self.icount > 1:
                ...
                self.normalizedDiffList[:, self.icount - 2] = self.diffList[:, self.icount - 2 ] / self.np.linalg.norm(self.diffList[:, self.icount - 2 ])
            ...
            if self.firstFull:
                ...
                self.normalizedDiffList[:, -1] = self.diffList[:, -1] / self.np.linalg.norm(self.diffList[:, -1])
            else: 
